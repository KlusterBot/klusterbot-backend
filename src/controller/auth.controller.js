...
class AuthControler {
    async login(res, payload) {
        ...
        if (Object.entries(payload).length === 0) {
            return sendResponse(res, 400, false, "failed to log In, missing payload.");
        }
        ...
        db.query({
            ...
            if (results.length === 0){
                const supports = await query("SELECT * FROM supports WHERE (email = ?)", [email]);
                ...
                if(supports.length !== 0){
                    ...
                    const user = (await query("SELECT * FROM users WHERE id = ?", [support.user_id]))[0];
    ...
    async register(res, payload) {
        ...
        if (Object.entries(payload).length === 0) {
            return sendResponse(res, 400, false, "Failed to register, missing payload.");
        }
        ...
        db.query({
            ...
            db.query({
                ...
                timeout: 40000,
                values: [userId || "", name || "",toHash(password) || "", email || "", "true", token],
                ...
                db.query({
                    ...
                    return sendResponse(res, 200, true, "Sign Up Successful", {
                        ...userPayload,
                        accessToken,
                    });
    ...
    async isLoggedIn(res, req){
        ...
        if (!tokens) {
            return res.status(400).json({ message: "Authorization header is required", error: true })
        }
        try {
